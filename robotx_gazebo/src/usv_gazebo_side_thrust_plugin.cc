...
{
  ROS_INFO("Loading usv_gazebo_side_thrust_plugin");
  model_ = _parent;
  ...
  if (!link_)
  {
    ROS_FATAL("usv_gazebo_side_thrust_plugin error: bodyName: %s does not exist\n", link_name_.c_str());
    return;
  }
  else
  {
  ...
  int argc = 0;
  char** argv = NULL;
  ros::init(argc, argv, "usv_side_thrust_gazebo",
	    ros::init_options::NoSigintHandler|ros::init_options::AnonymousName);
  rosnode_ = new ros::NodeHandle( node_namespace_ );

  cmd_drive_sub_ = rosnode_->subscribe("cmd_drive_side", 1, &UsvThrust::OnCmdDrive, this );
